...
function createSyncedManager(MicrosoftCalendarManager, upserted, contacts = []) {
...
test("_syncCalendar backfills stripped recurring occurrences from their series master", async () => {
  ...
  const upserted = [];
  const contacts = [];
  const manager = createSyncedManager(MicrosoftCalendarManager, upserted, contacts);
...
test("_syncCalendar keeps stripped occurrences when the series master fetch fails", async () => {
  const MicrosoftCalendarManager = loadManagerModule();
  const upserted = [];
  const manager = createSyncedManager(MicrosoftCalendarManager, upserted);
